...
void choose()
{
    ...
    OLED_Clear();
    OLED_ShowString(2,0,"Warehouse:");
    OLED_ShowNum(70, 0, count, 4, 16);
}
//限制函数
...
// 6个题60(easy)+40(difficult)
//主要部分怎么设置，主要代码+三题详细代码
//每一个内容都要看，原理性内容少，编程内容多，实验一定要做通
//1.MCU 相关概念，架构，MCU，CISC，RISC，432的图，特殊寄存器，有哪些外设模块
//2.GPIO 全记住
//3.时钟系统 7->5，怎样设置
//4.低功耗 每个模式怎样进入
//5.中断 每个概念都要清楚，都要记住，每个中断怎么设置,p1,adc,uart,timer
//6.定时器 结构图，计数模式，sys，wdt，溢出中断TAIE，捕获模式，PWM输出模块要会设（时钟，模式，中断，输出口根据数据手册），知道wdt有哪些作用
//7.ADC 总体结构图，设置参考电压，设置采样时钟，通道设置，RES分辨率，使能，sc=1，每个模块上的编程例子都要掌握
//8.UART 初始化帧格式设置，设置波特率，查表会算，查询，中断方式都要会，
