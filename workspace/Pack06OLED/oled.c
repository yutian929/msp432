...
void OLED_RST_Clr()
{MAP_GPIO_setOutputLowOnPin(GPIO_PORT_P6, GPIO_PIN1);}    //OLED_RST=0
void OLED_RST_Set()
{MAP_GPIO_setOutputHighOnPin(GPIO_PORT_P6, GPIO_PIN1);} //OLED_RST=1
void OLED_DC_Clr()
{MAP_GPIO_setOutputLowOnPin(GPIO_PORT_P6, GPIO_PIN0);}//OLED_DC=0
void OLED_DC_Set()
{MAP_GPIO_setOutputHighOnPin(GPIO_PORT_P6, GPIO_PIN0);}//OLED_DC=1
void OLED_SCLK_Clr()
{MAP_GPIO_setOutputLowOnPin(GPIO_PORT_P4, GPIO_PIN0);}//OLED_SCL=0
void OLED_SCLK_Set()
{MAP_GPIO_setOutputHighOnPin(GPIO_PORT_P4, GPIO_PIN0);}//OLED_SCL=1
void OLED_SDIN_Clr()
{MAP_GPIO_setOutputLowOnPin(GPIO_PORT_P3, GPIO_PIN0);}//OLED_SDIN=0
void OLED_SDIN_Set()
{MAP_GPIO_setOutputHighOnPin(GPIO_PORT_P3, GPIO_PIN0);}//OLED_SDIN=1;
...
void OLED_Fill(unsigned char fill_Data)
{
    ...
    for(m=0;m<8;m++)
    {
        ...
        OLED_WR_Byte(0x00,0);     //第四位起始地址
        OLED_WR_Byte(0x10,0);     //高四位起始地址
        for(n=0;n<128;n++)  //每页水平坐标为 0 - 127
            {
                OLED_WR_Byte(fill_Data,1); //写入数据 0x00 为全灭，0xff为全亮
            }
    }
}
