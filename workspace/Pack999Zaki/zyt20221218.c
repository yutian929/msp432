...
//SG90
void servo_init()//TIMERA1输出PWM波控制，信号口P74P75P76P77
{
    TIMER_A1->CTL = TIMER_A_CTL_TASSEL_1 | // ACLK
                         TIMER_A_CTL_MC_1|
                         TIMER_A_CTL_CLR; // up mode
    P7->DIR |= (BIT4+BIT5+BIT6+BIT7); // P2.6-7 option selection TA0.3-4
    P7->SEL0 |= (BIT4+BIT5+BIT6+BIT7);
    P7->SEL1 &=~(BIT4+BIT5+BIT6+BIT7);
}
void servo1_angle(int ang)//angle from 12~13~44~76~87
{
    // Configure GPIO
    TIMER_A1->CCR[0]=655;
...
void servo2_angle(int ang)//angle from 12~13~44~76~87
{
    // Configure GPIO
    TIMER_A1->CCR[0]=655;
    TIMER_A1->CCTL[5] = TIMER_A_CCTLN_OUTMOD_7;
    TIMER_A1->CCR[5]=ang;
}
void servo3_angle(int ang)//angle from 12~13~44~76~87
{
    // Configure GPIO
    TIMER_A1->CCR[0]=655;
    TIMER_A1->CCTL[6] = TIMER_A_CCTLN_OUTMOD_7;
    TIMER_A1->CCR[6]=ang;
}
void servo4_angle(int ang)//angle from 12~13~44~76~87
{
    // Configure GPIO
    TIMER_A1->CCR[0]=655;
    TIMER_A1->CCTL[7] = TIMER_A_CCTLN_OUTMOD_7;
    TIMER_A1->CCR[7]=ang;
}
//HCSR04超声
